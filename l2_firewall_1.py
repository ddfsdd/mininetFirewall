...
class LearningSwitch (object):
  ...
  def __init__ (self, connection, transparent):
    ...
    self.AddRule('00-00-00-00-00-01',EthAddr('00:00:00:00:00:02'),False)
    #log.debug("initiate switch %s",dpid_to_str(self.connection.dpid))
  ...
  def AddRule (self, dpidstr, src=0, value=True):
    ...
    log.debug("Adding dpidstr for switch %s and src %s tuple pair with value %s",dpidstr,src,value)
  def CheckRule (self, dpidstr, src=0):
    log.debug(" In check Rule src %s , dpidstr %s",src,dpidstr)
    try:
      entry = self.firewall[(dpidstr, src)]
      log.debug("src %s found in rule",src)
      if entry == True:
       log.debug("Rule:Forward")
      else:
       log.debug("Rule:Drop")
      return entry
    except KeyError:
      log.debug("No entry, dropping")
      return False
  def _handle_PacketIn (self, event):
    ...
    log.debug("src from %s , switch %s",packet.src,dpidstr)
    if self.CheckRule(dpidstr,packet.src) == False:
      log.debug("Firewall activated")
      drop()
      return
